...
const char * rootHTML = "<!DOCTYPE html><html lang=\"en\"><head></head></html>";
/*
const char rootHTML[] PROGMEM = R"=====(
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SHT Controller</title><style>body{text-align:center;font-size:24px;margin:0;padding:0;height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#121212;color:#fff}button{font-size:20px;padding:10px 20px;cursor:pointer;background:#4caf50;color:#fff;border:0;border-radius:5px;transition:.3s}button:hover{background:#388e3c}.status-container{display:flex;gap:30px;margin:20px 0}.status-box{padding:20px;border:2px solid #444;border-radius:10px;min-width:150px}.control-row{display:flex;gap:10px;margin:15px 0;justify-content:center;align-items:center}#fan-status{width:20px;height:20px;border-radius:50%;margin-top:10px}#fan-status.fan-on{background:#4caf50}#fan-status.fan-off{background:#ff4444}#manual-mode{padding:5px 10px;border-radius:5px}#settings-icon{position:absolute;top:20px;right:20px;font-size:30px;cursor:pointer;color:#fff}#settings-popup{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#333;padding:20px;border-radius:10px;color:#fff;width:300px;text-align:center}input{background:#333;color:#fff;border:1px solid #555;border-radius:5px;padding:15px;width:200px;margin:10px 0}</style></head><body><h2>SHT Controller</h2><div class="status-container"><div class="status-box">💧 Humidity<div id="humidity-value">--%</div></div><div class="status-box">🌡️ Temp<div id="tempC-value">--°C</div></div><div class="status-box">🌀 Fan<div id="fan-status" class="fan-off"></div></div></div><div class="control-row"><span id="manual-mode" style="background:#ff4444">Auto</span><input type="number" id="vent-time" placeholder="90" value="90"><button onclick="handleVent()">VENT</button></div><div class="control-row"><input type="number" step="0.1" id="threshold" placeholder="Humidity Threshold"><button onclick="updateThreshold(event)">Set</button></div><div class="control-row"><input type="number" id="min-vent-time" placeholder="Min Vent Time (s)"><button onclick="updateMinVentTime(event)">Set</button></div><div id="settings-icon" onclick="showSettingsPopup()">⚙️</div><div id="settings-popup"><h3>Settings</h3><p>Wi-Fi Configurations</p><form onsubmit="submitWifiConfig(event)"><input type="text" id="ssid" placeholder="SSID" required maxlength="32"><input type="password" id="passkey" placeholder="Passkey" required maxlength="32"><button type="submit">Save Configuration</button></form><button onclick="closeSettingsPopup()" style="margin-top:15px">Close</button></div><script>function updateStatus(){fetch('/state').then(r=>r.json()).then(data=>{document.getElementById('humidity-value').textContent=`${data.humidity.toFixed(1)}%`;document.getElementById('tempC-value').textContent=`${data.tempC.toFixed(1)}°C`;document.getElementById('fan-status').className=data.fan_state?'fan-on':'fan-off';document.getElementById('manual-mode').textContent=data.manual_mode?'Manual':'Auto';document.getElementById('manual-mode').style.backgroundColor=data.manual_mode?'#4caf50':'#ff4444'}).catch(console.error)}setInterval(updateStatus,500);function handleVent(){const e=document.getElementById('vent-time').value||90;fetch(`/vent?seconds=${e}`,{method:'PUT'}).then(e=>e.ok&&alert('Vent command sent!')).catch(console.error)}function updateThreshold(e){e.preventDefault(),fetch(`/threshold?value=${document.getElementById('threshold').value}`,{method:'PUT'}).then(e=>e.ok&&alert('Threshold updated!')).catch(console.error)}function updateMinVentTime(e){e.preventDefault(),fetch(`/min-vent-time?value=${document.getElementById('min-vent-time').value}`,{method:'PUT'}).then(e=>e.ok&&alert('Vent time updated!')).catch(console.error)}function showSettingsPopup(){document.getElementById('settings-popup').style.display='block'}function closeSettingsPopup(){document.getElementById('settings-popup').style.display='none'}function submitWifiConfig(e){e.preventDefault();const t=document.getElementById('ssid').value,n=document.getElementById('passkey').value;t.length>32||n.length>32?alert('Max 32 characters!'):fetch('/wifi-config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid:t,passkey:n})}).then(e=>alert(e.ok?'Saved!':'Failed!')).catch(console.error)}</script></body></html>
)=====";
*/
